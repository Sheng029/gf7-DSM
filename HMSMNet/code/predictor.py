...
class Predictor:
    ...
    def normImage(image1, image2):
        ...
        dx = sig.convolve2d(norm_image1, kx, 'same')
        dy = sig.convolve2d(norm_image2, ky, 'same')
        ...
        return norm_image1, norm_image2, dx, dy
    ...
    def __predict8x(self):
        ...
        while i + height <= rows:
            while j + width <= cols:
                patch1, patch2, dx, dy = self.normImage(image1[i:i+height, j:j+width], image2[i:i+height, j:j+width])
                dsp = self.model.predict([patch1, patch2, dx, dy])[0, :, :, 0]
                if i == 0 and j == 0:
        ...
        i = 0
        while i + height <= rows:
            patch1, patch2, dx, dy = self.normImage(image1[i:i+height, cols-width:cols], image2[i:i+height, cols-width:cols])
            dsp = self.model.predict([patch1, patch2, dx, dy])[0, :, :, 0]
            if i == 0:
        ...
        j = 0
        while j + width <= cols:
            patch1, patch2, dx, dy = self.normImage(image1[rows-height:rows, j:j+width], image2[rows-height:rows, j:j+width])
            dsp = self.model.predict([patch1, patch2, dx, dy])[0, :, :, 0]
            if j == 0:
            ...
            j += width // 2
        # 右下角
        patch1, patch2, dx, dy = self.normImage(image1[rows-height:rows, cols-width:cols], image2[rows-height:rows, cols-width:cols])
        dsp = self.model.predict([patch1, patch2, dx, dy])[0, :, :, 0]
        ...
        while i + height <= rows:
            while j + width <= cols:
                ...
                if i == 0 and j == 0:
                    ...
                    patch1, patch2, dx, dy = self.normImage(patch1, patch2)
                    dsp = self.model.predict([patch1, patch2, dx, dy])[0, :, :, 0]
                    ...
                    if j - avg > 0 and j - avg + width < cols:
                        ...
                        patch1, patch2, dx, dy = self.normImage(patch1, patch2)
                        dsp = self.model.predict([patch1, patch2, dx, dy])[0, :, :, 0] + avg
...
if __name__ == '__main__':
    ...
    model_dir = '/home/hesheng/Networks/HMSMNet/model/HMSMNetMIX2048P'
