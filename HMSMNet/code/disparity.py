...
def normImage(left_patch, right_patch):
    ...
    dx = sig.convolve2d(left_patch, kx, 'same')
    dy = sig.convolve2d(right_patch, ky, 'same')
    ...
    return left_patch, right_patch, dx, dy
...
def genDisparityMap(left_img_path, right_img_path, height, width, model_dir):
    ...
    while i + height <= rows:
        while j + width <= cols:
            ...
            left_patch, right_patch, dx, dy = normImage(left_patch, right_patch)
            disp = model.predict([left_patch, right_patch, dx, dy])[0, :, :, 0]
            if i == 0 and j == 0:
